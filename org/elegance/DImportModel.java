...
public class DImportModel extends AbstractTableModel {
	...
	public DImportModel(DElement fielddef) {
    ...
    public Object getValueAt(int aRow, int aColumn) {
        Vector<String> row = rows.elementAt(aRow);
	...
	public void gettextdata(JSplitPane panel, String delimiter) { // Get all rows.
	...
	public void getrecorddata(JSplitPane panel) {
		// Get all rows .......
		FileReader input = null;
		String filename = "";
		rows.removeAllElements();
	...
	public void getexceldata(JSplitPane panel, String worksheet) { // Get all rows.
		String filename = "";
		rows.removeAllElements();
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			for(i = sheet.getFirstRowNum()+1; i <= sheet.getLastRowNum(); i++) {
				...
				if(row!=null)  {
					...
					//System.out.println("EXCEL ROW \t" + myvec);
					for (int j=0;j<getColumnCount();j++)
	...
	public void clearupload() {
		System.out.println("DEBUG: At DImportModel:clearupload()");
		rows.removeAllElements();
		fireTableChanged(null); // Tell the listeners a new table has arrived.
		}
